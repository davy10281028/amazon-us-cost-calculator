...
| **自動估算** | FBA 配送費（Size Tier + 體積重 + 售價三檔 + **服裝/非服裝** + **旺季/非旺季** + 燃油附加費）、月倉儲費（**標準/大件分開**）、Inbound Placement、階梯佣金（含 **$0.30 最低佣金**）、退款管理費 |
...
├── docs/
│   └── UPDATING-RATES.md   # ★ 每季更新的逐項對照清單（含如何讀 Seller Central）
├── scripts/
│   └── read-seller-central.py  # 用 Lens + Midway 讀需登入的官方費率頁
├── test/
│   ├── engine.test.js      # 209 項斷言：官方範例驗算 + bug 迴歸 + 公式驗算 + 端到端
...
```bash
# 引擎測試：官方範例驗算 + bug 迴歸 + 公式驗算 + 端到端
node test/engine.test.js

# 靜態檢查：i18n key、element id、中英字典對齊
node test/lint.js

# 讀取需要登入的官方費率頁（需先 mwinit -f）
python scripts/read-seller-central.py GABBX6GZPA8MSZGW --show
```

`engine.test.js` 的 A 區塊拿官方 `GABBX6GZPA8MSZGW`「Product size examples」
的 6 個商品範例 × 非旺季/旺季 = **12 個官方數字**當黃金基準，比對到分為止。
這同時驗證了 size tier 判定、體積重、售價分檔、服裝/非服裝費率卡、旺季費率卡、
Bulky/XL 的整磅取整級距，以及 XL 150+ lb 的實重例外。

其餘測試都是從 `rates.js` 讀值再驗算（公式、上限、迴歸、端到端），
任何費率版本都會跑；與原版 v1.1 的 parity 比對則依各組 `_verified` 自動跳過。
...
| 13 | **FBA 費率表抄錯欄位** —— 官方表把 2025 與 2026 兩個期間並排放在同一張表，原版把 2025 年的欄位當成 `low`/`mid`，又把 2026 年的 `$10-$50` 當成 `>$50` | **最常見的 $10-$50 售價區間一直在用 2025 年舊費率**。也是「Small Bulky 的 >$50 檔比 $10-$50 便宜」那個矛盾的真正原因 |
| 14 | **Bulky / Extra-Large 的級距用連續乘** —— 官方是 `ceil(計費重) - freeLb` 個整磅級距 | 官方 Baby cot 範例 7.90 lb 應為 7 級距 = $10.21，原版算 6.9 級距 = $10.17 |
| 15 | **沒有旺季配送費** —— 官方旺季 2026-10-15 ~ 2027-01-14，費用平均高 5-10% | 距今 7 週；旺季期間全面低估 |
| 16 | **服裝品類用非服裝費率** —— 官方有獨立的服裝費率卡（3+lb 級距是每半磅 $0.16 而非每 4 oz $0.08） | 服飾賣家的 FBA 費用低估，官方 T-shirt 範例差 $0.57 |
...
- **測試**：209 項斷言（含 12 個官方費率範例驗算） + 靜態 lint；費率 parity 會依 `meta.version` 自動跳過，不會假失敗
